...
// Calculate date when a sigma band reaches a target price
...
// Populate milestone price table with all sigma bands
function populateMilestoneTable() {
  ...
  for (const price of MILESTONE_PRICES) {
    ...
    for (const k of bands) {
      ...
      const label = isPast ? 'Past' : PowerLaw.formatDate(date);
